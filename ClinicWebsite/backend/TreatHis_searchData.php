...
$sql = "SELECT * FROM Treatmenthistory WHERE AnimalID LIKE '%$animal%' ORDER BY AnimalID ASC";
   ...
   <div id="boxform">
         ...
         <div class="c" id="gbox">
            ...
            <p style="font-size: 1.4em; color: #265a66;">
               <b>&emsp;&emsp;&emsp;&emsp;&emsp;<font size="6">ผลการค้นหาข้อมูลการนัดหมาย</font></b>
            </p>
         </div>
         <br>

   
    <h1 style="color: #265a66;  class="text-center" >&emsp;ระบบข้อมูลคลินิกรักษาสัตว์</h1>
   
    <?php if($count>0){?>
        <form action="TreatHis_searchData.php"  class="form-group" method="POST">
        <label for="">&emsp;ค้นหาข้อมูลการนัดหมาย</label>
        <input type="text" placeholder="ป้อนIDสัตว์เลี้ยง" name="idsus" class="form-control">
    ...
    <?php } else {?>
        ...
        <h2 class="text-right">&emsp;ไม่มีข้อมูลพนักงานในฐานข้อมูล</h2>
        </div>
        <?php } ?>
    ...
    <?php if($count>0){?>
        <button style=" border-radius: 50px; padding: 0.4em 1em; color:  #e6e6e6; background-color:#f1a470 ;font-size: 1.5em; border-color:transparent;  font-family: 'Kanit', sans-serif;" onclick="checkAll()">เลือกทั้งหมด</button>
